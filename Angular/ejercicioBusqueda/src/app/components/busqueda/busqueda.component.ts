...
export class BusquedaComponent {
  ...
  aniadirAlumno(nombrePasado:string,apellidoPasado:string,edadPasado:string,cicloPasado:string){
    this.alumno = {
      nombre: nombrePasado,
  ...
  cambiarnumero(ciclo:string,estudios:string){
    if (this.filtro == 1) {
      this.numAlumnosEncontrados =  this.alumnos.filter((item) => item.ciclo === ciclo).length
    }else{
      this.numAlumnosEncontrados =  this.alumnos.filter((item) => item.estudios===estudios).length
    }
  }
